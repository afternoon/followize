...
fw.state = {
    ...
    update: function(users) {
        for (var i = 0; i < users.length; i++) {
            fw.state._users["user" + users[i].screen_name] = users[i];
        }
...
fw.view = {
    ...
    STATUS_HTML: '<div class="tweet"><span class="text">{{{html}}}</span> <span class="created_at">{{created_at}}</span> <span class="source">from {{{source}}}</span></div>',
    ...
    show: function(users) {
        ...
        $.map(users, function(user, i) { fw.view.appendUser(fw.util.addLinksToUser(user), tbody); });
    },
